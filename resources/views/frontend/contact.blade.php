@extends('layouts.frontend.master')

@section('meta')

@endsection

@section('css')

@endsection

@section('content')
 <main class="main-area fix">

        <!-- breadcrumb-area -->
        <section class="breadcrumb__area">
            <div class="container">
                <div class="row justify-content-center">
                    <div class="col-lg-8">
                        <div class="breadcrumb__content">
                            <nav class="breadcrumb">
                                <span property="itemListElement" typeof="ListItem">
                                    <a href="index.html">Home</a>
                                </span>
                                <span class="breadcrumb-separator">//</span>
                                <span property="itemListElement" typeof="ListItem">Contact</span>
                            </nav>
                            <h2 class="title">Have Questions? Our Support Team Is Always Ready</h2>
                        </div>
                    </div>
                </div>
            </div>
            <div class="shape">
                <img src="assets/img/images/breadcrumb_shape.png" alt="">
            </div>
        </section>
        <!-- breadcrumb-area-end -->

        <!-- contact-area -->
        <section class="contact__area section-pb-120">
            <div class="container">
                <div class="row gutter-y-30">
                    <div class="col-lg-6">
                        <div class="contact__info-box">
                            <h3 class="title">Get In Touch</h3>
                            <p>We provide reliable, secure, and high-performance hosting solutions tailored to your needs, ensuring fast, scalable, and hassle-free online experiences.</p>
                            <ul class="list-wrap">
                                <li>
                                    <div class="icon">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M3 7C3 6.46957 3.21071 5.96086 3.58579 5.58579C3.96086 5.21071 4.46957 5 5 5H19C19.5304 5 20.0391 5.21071 20.4142 5.58579C20.7893 5.96086 21 6.46957 21 7V17C21 17.5304 20.7893 18.0391 20.4142 18.4142C20.0391 18.7893 19.5304 19 19 19H5C4.46957 19 3.96086 18.7893 3.58579 18.4142C3.21071 18.0391 3 17.5304 3 17V7Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                            <path d="M3 7L12 13L21 7" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </div>
                                    <a href="mailto:sayhello@DeveonHost.com">sayhello@DeveonHost.com</a>
                                </li>
                                <li>
                                    <div class="icon">
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M9 11C9 11.7956 9.31607 12.5587 9.87868 13.1213C10.4413 13.6839 11.2044 14 12 14C12.7956 14 13.5587 13.6839 14.1213 13.1213C14.6839 12.5587 15 11.7956 15 11C15 10.2044 14.6839 9.44129 14.1213 8.87868C13.5587 8.31607 12.7956 8 12 8C11.2044 8 10.4413 8.31607 9.87868 8.87868C9.31607 9.44129 9 10.2044 9 11Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                            <path d="M17.657 16.6567L13.414 20.8997C13.039 21.2743 12.5306 21.4848 12.0005 21.4848C11.4704 21.4848 10.962 21.2743 10.587 20.8997L6.343 16.6567C5.22422 15.5379 4.46234 14.1124 4.15369 12.5606C3.84504 11.0087 4.00349 9.40022 4.60901 7.93844C5.21452 6.47665 6.2399 5.22725 7.55548 4.34821C8.87107 3.46918 10.4178 3 12 3C13.5822 3 15.1289 3.46918 16.4445 4.34821C17.7601 5.22725 18.7855 6.47665 19.391 7.93844C19.9965 9.40022 20.155 11.0087 19.8463 12.5606C19.5377 14.1124 18.7758 15.5379 17.657 16.6567Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </div>
                                    <span>1454 Street, North Valley, San Francisco, California</span>
                                </li>
                            </ul>
                            <div class="header-dropdown-social contact__social">
                                <span class="title">Get Connected</span>
                                <ul class="list-wrap">
                                    <li>
                                        <a href="https://www.facebook.com/" target="_blank"><i class="fab fa-facebook"></i></a>
                                    </li>
                                    <li>
                                        <a href="https://www.pinterest.com/" target="_blank"><i class="fab fa-pinterest"></i></a>
                                    </li>
                                    <li>
                                        <a href="https://github.com/" target="_blank"><i class="fab fa-github"></i></a>
                                    </li>
                                    <li>
                                        <a href="https://www.youtube.com/" target="_blank"><i class="fab fa-youtube"></i></a>
                                    </li>
                                </ul>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-6">
                        <div class="contact__form-box">
                            <h2 class="title">Contact</h2>
                            <form  id="contact-form" action="assets/mail.php" class="cantact__form" method="POST">
                                <div class="form-grp">
                                    <input type="text" name="name" placeholder="Name">
                                </div>
                                <div class="form-grp">
                                    <input type="email" name="email" placeholder="E-mail">
                                </div>
                                <div class="form-grp">
                                    <input type="text" name="subject" placeholder="Subject">
                                </div>
                                <div class="form-grp">
                                    <textarea name="message" placeholder="Message"></textarea>
                                </div>
                                <button type="submit" class="tg-btn tg-btn-two">
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path d="M6 5.33268V3.99935C6 3.64573 6.14048 3.30659 6.39052 3.05654C6.64057 2.80649 6.97971 2.66602 7.33333 2.66602H12C12.3536 2.66602 12.6928 2.80649 12.9428 3.05654C13.1929 3.30659 13.3333 3.64573 13.3333 3.99935V11.9993C13.3333 12.353 13.1929 12.6921 12.9428 12.9422C12.6928 13.1922 12.3536 13.3327 12 13.3327H7.33333C6.97971 13.3327 6.64057 13.1922 6.39052 12.9422C6.14048 12.6921 6 12.353 6 11.9993V10.666" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M2 8H10.6667L8.66667 6" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" />
                                        <path d="M8.66797 10L10.668 8" stroke="currentColor" stroke-width="1.2" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                    Submit Message
                                </button>
                            </form>
                            <p class="ajax-response mb-0"></p>
                        </div>
                    </div>
                </div>
                <div class="contact__item-wrap section-pt-120">
                    <div class="section__title text-center mb-60">
                        <h2 class="title">Need Assistance? Reach Out Anytime</h2>
                        <p>Are you ready to make your ideas a reality and improve your brand? I'm here to create and cooperate.</p>
                    </div>
                    <div class="row gutter-y-30">
                        <div class="col-lg-4 col-md-6">
                            <div class="contact__item">
                                <div class="contact__item-icon">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path opacity="0.12" d="M12 2C13.3132 2 14.6136 2.25866 15.8268 2.76121C17.0401 3.26375 18.1425 4.00035 19.0711 4.92893C19.9997 5.85752 20.7363 6.95991 21.2388 8.17317C21.7413 9.38643 22 10.6868 22 12L12 12L12 2Z" fill="currentColor" />
                                        <path d="M12 2C13.3132 2 14.6136 2.25866 15.8268 2.76121C17.0401 3.26375 18.1425 4.00035 19.0711 4.92893C19.9997 5.85752 20.7363 6.95991 21.2388 8.17317C21.7413 9.38643 22 10.6868 22 12M12 2V12M12 2C6.47715 2 2 6.47715 2 12C2 17.5228 6.47715 22 12 22C17.5228 22 22 17.5229 22 12M12 2C17.5228 2 22 6.47716 22 12M22 12L12 12M22 12C22 13.5781 21.6265 15.1338 20.9101 16.5399C20.1936 17.946 19.1546 19.1626 17.8779 20.0902L12 12" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                                <div class="contact__item-content">
                                    <h2 class="title">Sales</h2>
                                    <p>Our sales team is ready to discuss your needs and how our AI solutions can help.</p>
                                    <ul class="list-wrap">
                                        <li>
                                            <a href="mailto:sayhello@DeveonHost.com">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path opacity="0.12" d="M11.5266 10.5969C10.9757 10.9825 10.7002 11.1754 10.4005 11.2501C10.1358 11.3161 9.85898 11.3161 9.59428 11.2501C9.29463 11.1754 9.01914 10.9825 8.46816 10.5969L1.66406 5.83398C1.66406 4.45327 2.78335 3.33398 4.16406 3.33398H15.8307C17.2114 3.33398 18.3307 4.45327 18.3307 5.83398L11.5266 10.5969Z" fill="currentColor" />
                                                    <path d="M17.914 15.0007L12.3783 10.0007M7.61644 10.0007L2.08076 15.0007M1.66406 5.83398L8.46816 10.5969C9.01914 10.9825 9.29463 11.1754 9.59428 11.2501C9.85898 11.3161 10.1358 11.3161 10.4005 11.2501C10.7002 11.1754 10.9757 10.9825 11.5266 10.5969L18.3307 5.83398M5.66406 16.6673H14.3307C15.7309 16.6673 16.4309 16.6673 16.9657 16.3948C17.4361 16.1552 17.8186 15.7727 18.0582 15.3023C18.3307 14.7675 18.3307 14.0674 18.3307 12.6673V7.33398C18.3307 5.93385 18.3307 5.23379 18.0582 4.69901C17.8186 4.2286 17.4361 3.84615 16.9657 3.60647C16.4309 3.33398 15.7309 3.33398 14.3307 3.33398H5.66406C4.26393 3.33398 3.56387 3.33398 3.02909 3.60647C2.55868 3.84615 2.17623 4.2286 1.93655 4.69901C1.66406 5.23379 1.66406 5.93385 1.66406 7.33398V12.6673C1.66406 14.0674 1.66406 14.7675 1.93655 15.3023C2.17623 15.7727 2.55868 16.1552 3.02909 16.3948C3.56387 16.6673 4.26393 16.6673 5.66406 16.6673Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                                Email
                                            </a>
                                        </li>
                                        <li>
                                            <a href="tel:0123456789">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path opacity="0.12" d="M6.15153 8.21178C6.73153 9.41979 7.52219 10.552 8.5235 11.5533C9.52482 12.5546 10.657 13.3453 11.865 13.9253C11.9689 13.9752 12.0209 14.0001 12.0866 14.0193C12.3203 14.0874 12.6072 14.0385 12.805 13.8968C12.8607 13.8569 12.9083 13.8093 13.0036 13.714C13.2949 13.4227 13.4406 13.277 13.5871 13.1818C14.1395 12.8226 14.8516 12.8226 15.404 13.1818C15.5505 13.277 15.6962 13.4227 15.9875 13.714L16.1499 13.8764C16.5928 14.3193 16.8142 14.5407 16.9345 14.7785C17.1737 15.2515 17.1737 15.8101 16.9345 16.283C16.8142 16.5208 16.5928 16.7423 16.1499 17.1852L16.0186 17.3165C15.5772 17.7579 15.3565 17.9785 15.0565 18.1471C14.7236 18.3341 14.2065 18.4686 13.8246 18.4674C13.4805 18.4664 13.2453 18.3997 12.775 18.2661C10.2471 17.5487 7.86184 16.1949 5.87185 14.205C3.88187 12.215 2.52814 9.82968 1.81067 7.30185C1.67716 6.83148 1.61041 6.59629 1.60938 6.25217C1.60825 5.87032 1.74271 5.35324 1.92973 5.02032C2.09827 4.72029 2.31895 4.49961 2.7603 4.05826L2.89166 3.9269C3.33453 3.48403 3.55596 3.2626 3.79378 3.14231C4.26675 2.90308 4.82531 2.90308 5.29827 3.14231C5.53609 3.2626 5.75752 3.48403 6.20039 3.9269L6.36278 4.08929C6.65412 4.38063 6.79979 4.5263 6.89503 4.67278C7.25419 5.22519 7.25419 5.93734 6.89503 6.48975C6.79979 6.63623 6.65412 6.7819 6.36278 7.07324C6.26752 7.1685 6.21989 7.21613 6.18003 7.2718C6.03835 7.46966 5.98943 7.75655 6.05754 7.99017C6.0767 8.05592 6.10164 8.10787 6.15153 8.21178Z" fill="currentColor" />
                                                    <path d="M11.7094 4.99935C12.5233 5.15815 13.2713 5.55623 13.8577 6.14263C14.4441 6.72903 14.8422 7.47707 15.001 8.29102M11.7094 1.66602C13.4004 1.85388 14.9773 2.61116 16.1812 3.81352C17.3851 5.01588 18.1444 6.59186 18.3344 8.28268M8.5235 11.5519C7.52219 10.5506 6.73153 9.41839 6.15153 8.21037C6.10164 8.10647 6.0767 8.05451 6.05753 7.98877C5.98943 7.75514 6.03835 7.46826 6.18003 7.2704C6.21989 7.21472 6.26752 7.16709 6.36278 7.07183C6.65412 6.78049 6.79979 6.63483 6.89503 6.48834C7.25419 5.93593 7.25419 5.22378 6.89503 4.67138C6.79979 4.52489 6.65412 4.37923 6.36278 4.08789L6.20039 3.92549C5.75752 3.48263 5.53609 3.26119 5.29827 3.1409C4.8253 2.90168 4.26675 2.90168 3.79378 3.1409C3.55596 3.26119 3.33453 3.48263 2.89166 3.92549L2.7603 4.05686C2.31895 4.49821 2.09827 4.71889 1.92973 5.01891C1.74271 5.35183 1.60825 5.86891 1.60938 6.25076C1.61041 6.59488 1.67716 6.83007 1.81067 7.30044C2.52814 9.82827 3.88187 12.2136 5.87185 14.2036C7.86184 16.1935 10.2471 17.5473 12.775 18.2647C13.2453 18.3983 13.4805 18.465 13.8246 18.466C14.2065 18.4672 14.7236 18.3327 15.0565 18.1457C15.3565 17.9771 15.5772 17.7565 16.0186 17.3151L16.1499 17.1837C16.5928 16.7409 16.8142 16.5194 16.9345 16.2816C17.1737 15.8087 17.1737 15.2501 16.9345 14.7771C16.8142 14.5393 16.5928 14.3179 16.1499 13.875L15.9875 13.7126C15.6962 13.4213 15.5505 13.2756 15.404 13.1804C14.8516 12.8212 14.1395 12.8212 13.5871 13.1804C13.4406 13.2756 13.2949 13.4213 13.0036 13.7126C12.9083 13.8079 12.8607 13.8555 12.805 13.8954C12.6072 14.0371 12.3203 14.086 12.0866 14.0179C12.0209 13.9987 11.9689 13.9738 11.865 13.9239C10.657 13.3439 9.52482 12.5532 8.5235 11.5519Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                  </svg>
                                                Call
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-6">
                            <div class="contact__item">
                                <div class="contact__item-icon">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <path opacity="0.12" d="M3 7.8C3 6.11984 3 5.27976 3.32698 4.63803C3.6146 4.07354 4.07354 3.6146 4.63803 3.32698C5.27976 3 6.11984 3 7.8 3H16.2C17.8802 3 18.7202 3 19.362 3.32698C19.9265 3.6146 20.3854 4.07354 20.673 4.63803C21 5.27976 21 6.11984 21 7.8V16.2C21 17.8802 21 18.7202 20.673 19.362C20.3854 19.9265 19.9265 20.3854 19.362 20.673C18.7202 21 17.8802 21 16.2 21H7.8C6.11984 21 5.27976 21 4.63803 20.673C4.07354 20.3854 3.6146 19.9265 3.32698 19.362C3 18.7202 3 17.8802 3 16.2V7.8Z" fill="currentColor" />
                                        <path d="M17 9L11.5657 14.4343C11.3677 14.6323 11.2687 14.7313 11.1545 14.7684C11.0541 14.8011 10.9459 14.8011 10.8455 14.7684C10.7313 14.7313 10.6323 14.6323 10.4343 14.4343L8.56569 12.5657C8.36768 12.3677 8.26867 12.2687 8.15451 12.2316C8.05409 12.199 7.94591 12.199 7.84549 12.2316C7.73133 12.2687 7.63232 12.3677 7.43432 12.5657L3 17M17 9H13M17 9V13M7.8 21H16.2C17.8802 21 18.7202 21 19.362 20.673C19.9265 20.3854 20.3854 19.9265 20.673 19.362C21 18.7202 21 17.8802 21 16.2V7.8C21 6.11984 21 5.27976 20.673 4.63803C20.3854 4.07354 19.9265 3.6146 19.362 3.32698C18.7202 3 17.8802 3 16.2 3H7.8C6.11984 3 5.27976 3 4.63803 3.32698C4.07354 3.6146 3.6146 4.07354 3.32698 4.63803C3 5.27976 3 6.11984 3 7.8V16.2C3 17.8802 3 18.7202 3.32698 19.362C3.6146 19.9265 4.07354 20.3854 4.63803 20.673C5.27976 21 6.11984 21 7.8 21Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                                <div class="contact__item-content">
                                    <h2 class="title">Development</h2>
                                    <p>Our Development team is ready to discuss your needs and how our AI solutions can help.</p>
                                    <ul class="list-wrap">
                                        <li>
                                            <a href="mailto:sayhello@DeveonHost.com">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path opacity="0.12" d="M11.5266 10.5969C10.9757 10.9825 10.7002 11.1754 10.4005 11.2501C10.1358 11.3161 9.85898 11.3161 9.59428 11.2501C9.29463 11.1754 9.01914 10.9825 8.46816 10.5969L1.66406 5.83398C1.66406 4.45327 2.78335 3.33398 4.16406 3.33398H15.8307C17.2114 3.33398 18.3307 4.45327 18.3307 5.83398L11.5266 10.5969Z" fill="currentColor" />
                                                    <path d="M17.914 15.0007L12.3783 10.0007M7.61644 10.0007L2.08076 15.0007M1.66406 5.83398L8.46816 10.5969C9.01914 10.9825 9.29463 11.1754 9.59428 11.2501C9.85898 11.3161 10.1358 11.3161 10.4005 11.2501C10.7002 11.1754 10.9757 10.9825 11.5266 10.5969L18.3307 5.83398M5.66406 16.6673H14.3307C15.7309 16.6673 16.4309 16.6673 16.9657 16.3948C17.4361 16.1552 17.8186 15.7727 18.0582 15.3023C18.3307 14.7675 18.3307 14.0674 18.3307 12.6673V7.33398C18.3307 5.93385 18.3307 5.23379 18.0582 4.69901C17.8186 4.2286 17.4361 3.84615 16.9657 3.60647C16.4309 3.33398 15.7309 3.33398 14.3307 3.33398H5.66406C4.26393 3.33398 3.56387 3.33398 3.02909 3.60647C2.55868 3.84615 2.17623 4.2286 1.93655 4.69901C1.66406 5.23379 1.66406 5.93385 1.66406 7.33398V12.6673C1.66406 14.0674 1.66406 14.7675 1.93655 15.3023C2.17623 15.7727 2.55868 16.1552 3.02909 16.3948C3.56387 16.6673 4.26393 16.6673 5.66406 16.6673Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                                Email
                                            </a>
                                        </li>
                                        <li>
                                            <a href="tel:0123456789">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path opacity="0.12" d="M6.15153 8.21178C6.73153 9.41979 7.52219 10.552 8.5235 11.5533C9.52482 12.5546 10.657 13.3453 11.865 13.9253C11.9689 13.9752 12.0209 14.0001 12.0866 14.0193C12.3203 14.0874 12.6072 14.0385 12.805 13.8968C12.8607 13.8569 12.9083 13.8093 13.0036 13.714C13.2949 13.4227 13.4406 13.277 13.5871 13.1818C14.1395 12.8226 14.8516 12.8226 15.404 13.1818C15.5505 13.277 15.6962 13.4227 15.9875 13.714L16.1499 13.8764C16.5928 14.3193 16.8142 14.5407 16.9345 14.7785C17.1737 15.2515 17.1737 15.8101 16.9345 16.283C16.8142 16.5208 16.5928 16.7423 16.1499 17.1852L16.0186 17.3165C15.5772 17.7579 15.3565 17.9785 15.0565 18.1471C14.7236 18.3341 14.2065 18.4686 13.8246 18.4674C13.4805 18.4664 13.2453 18.3997 12.775 18.2661C10.2471 17.5487 7.86184 16.1949 5.87185 14.205C3.88187 12.215 2.52814 9.82968 1.81067 7.30185C1.67716 6.83148 1.61041 6.59629 1.60938 6.25217C1.60825 5.87032 1.74271 5.35324 1.92973 5.02032C2.09827 4.72029 2.31895 4.49961 2.7603 4.05826L2.89166 3.9269C3.33453 3.48403 3.55596 3.2626 3.79378 3.14231C4.26675 2.90308 4.82531 2.90308 5.29827 3.14231C5.53609 3.2626 5.75752 3.48403 6.20039 3.9269L6.36278 4.08929C6.65412 4.38063 6.79979 4.5263 6.89503 4.67278C7.25419 5.22519 7.25419 5.93734 6.89503 6.48975C6.79979 6.63623 6.65412 6.7819 6.36278 7.07324C6.26752 7.1685 6.21989 7.21613 6.18003 7.2718C6.03835 7.46966 5.98943 7.75655 6.05754 7.99017C6.0767 8.05592 6.10164 8.10787 6.15153 8.21178Z" fill="currentColor" />
                                                    <path d="M11.7094 4.99935C12.5233 5.15815 13.2713 5.55623 13.8577 6.14263C14.4441 6.72903 14.8422 7.47707 15.001 8.29102M11.7094 1.66602C13.4004 1.85388 14.9773 2.61116 16.1812 3.81352C17.3851 5.01588 18.1444 6.59186 18.3344 8.28268M8.5235 11.5519C7.52219 10.5506 6.73153 9.41839 6.15153 8.21037C6.10164 8.10647 6.0767 8.05451 6.05753 7.98877C5.98943 7.75514 6.03835 7.46826 6.18003 7.2704C6.21989 7.21472 6.26752 7.16709 6.36278 7.07183C6.65412 6.78049 6.79979 6.63483 6.89503 6.48834C7.25419 5.93593 7.25419 5.22378 6.89503 4.67138C6.79979 4.52489 6.65412 4.37923 6.36278 4.08789L6.20039 3.92549C5.75752 3.48263 5.53609 3.26119 5.29827 3.1409C4.8253 2.90168 4.26675 2.90168 3.79378 3.1409C3.55596 3.26119 3.33453 3.48263 2.89166 3.92549L2.7603 4.05686C2.31895 4.49821 2.09827 4.71889 1.92973 5.01891C1.74271 5.35183 1.60825 5.86891 1.60938 6.25076C1.61041 6.59488 1.67716 6.83007 1.81067 7.30044C2.52814 9.82827 3.88187 12.2136 5.87185 14.2036C7.86184 16.1935 10.2471 17.5473 12.775 18.2647C13.2453 18.3983 13.4805 18.465 13.8246 18.466C14.2065 18.4672 14.7236 18.3327 15.0565 18.1457C15.3565 17.9771 15.5772 17.7565 16.0186 17.3151L16.1499 17.1837C16.5928 16.7409 16.8142 16.5194 16.9345 16.2816C17.1737 15.8087 17.1737 15.2501 16.9345 14.7771C16.8142 14.5393 16.5928 14.3179 16.1499 13.875L15.9875 13.7126C15.6962 13.4213 15.5505 13.2756 15.404 13.1804C14.8516 12.8212 14.1395 12.8212 13.5871 13.1804C13.4406 13.2756 13.2949 13.4213 13.0036 13.7126C12.9083 13.8079 12.8607 13.8555 12.805 13.8954C12.6072 14.0371 12.3203 14.086 12.0866 14.0179C12.0209 13.9987 11.9689 13.9738 11.865 13.9239C10.657 13.3439 9.52482 12.5532 8.5235 11.5519Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                  </svg>
                                                Call
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                        <div class="col-lg-4 col-md-6">
                            <div class="contact__item">
                                <div class="contact__item-icon">
                                    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        <g opacity="0.12">
                                          <path d="M18.5 13.5C20.433 13.5 22 11.933 22 10C22 8.067 20.433 6.5 18.5 6.5V13.5Z" fill="currentColor" />
                                          <path d="M9.99999 5.5H6.5C4.01472 5.5 2 7.51472 2 10C2 12.4853 4.01472 14.5 6.5 14.5H10L9.99999 5.5Z" fill="currentColor" />
                                        </g>
                                        <path d="M4 14.0004L5.57465 20.299C5.61893 20.4761 5.64107 20.5647 5.66727 20.642C5.92317 21.3975 6.60352 21.9287 7.39852 21.9937C7.4799 22.0004 7.5712 22.0004 7.75379 22.0004C7.98244 22.0004 8.09677 22.0004 8.19308 21.9911C9.145 21.8987 9.89834 21.1454 9.99066 20.1935C10 20.0972 10 19.9828 10 19.7542V5.5004M18.5 13.5004C20.433 13.5004 22 11.9334 22 10.0004C22 8.0674 20.433 6.5004 18.5 6.5004M10.25 5.5004H6.5C4.01472 5.5004 2 7.51512 2 10.0004C2 12.4857 4.01472 14.5004 6.5 14.5004H10.25C12.0164 14.5004 14.1772 15.4473 15.8443 16.356C16.8168 16.8862 17.3031 17.1513 17.6216 17.1123C17.9169 17.0761 18.1402 16.9435 18.3133 16.7015C18.5 16.4405 18.5 15.9184 18.5 14.8741V5.12668C18.5 4.0824 18.5 3.56025 18.3133 3.29929C18.1402 3.0573 17.9169 2.9247 17.6216 2.88853C17.3031 2.84952 16.8168 3.1146 15.8443 3.64475C14.1772 4.55351 12.0164 5.5004 10.25 5.5004Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                                    </svg>
                                </div>
                                <div class="contact__item-content">
                                    <h2 class="title">Media</h2>
                                    <p>Our Media team is ready to discuss your needs and how our AI solutions can help.</p>
                                    <ul class="list-wrap">
                                        <li>
                                            <a href="mailto:sayhello@DeveonHost.com">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path opacity="0.12" d="M11.5266 10.5969C10.9757 10.9825 10.7002 11.1754 10.4005 11.2501C10.1358 11.3161 9.85898 11.3161 9.59428 11.2501C9.29463 11.1754 9.01914 10.9825 8.46816 10.5969L1.66406 5.83398C1.66406 4.45327 2.78335 3.33398 4.16406 3.33398H15.8307C17.2114 3.33398 18.3307 4.45327 18.3307 5.83398L11.5266 10.5969Z" fill="currentColor" />
                                                    <path d="M17.914 15.0007L12.3783 10.0007M7.61644 10.0007L2.08076 15.0007M1.66406 5.83398L8.46816 10.5969C9.01914 10.9825 9.29463 11.1754 9.59428 11.2501C9.85898 11.3161 10.1358 11.3161 10.4005 11.2501C10.7002 11.1754 10.9757 10.9825 11.5266 10.5969L18.3307 5.83398M5.66406 16.6673H14.3307C15.7309 16.6673 16.4309 16.6673 16.9657 16.3948C17.4361 16.1552 17.8186 15.7727 18.0582 15.3023C18.3307 14.7675 18.3307 14.0674 18.3307 12.6673V7.33398C18.3307 5.93385 18.3307 5.23379 18.0582 4.69901C17.8186 4.2286 17.4361 3.84615 16.9657 3.60647C16.4309 3.33398 15.7309 3.33398 14.3307 3.33398H5.66406C4.26393 3.33398 3.56387 3.33398 3.02909 3.60647C2.55868 3.84615 2.17623 4.2286 1.93655 4.69901C1.66406 5.23379 1.66406 5.93385 1.66406 7.33398V12.6673C1.66406 14.0674 1.66406 14.7675 1.93655 15.3023C2.17623 15.7727 2.55868 16.1552 3.02909 16.3948C3.56387 16.6673 4.26393 16.6673 5.66406 16.6673Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                </svg>
                                                Email
                                            </a>
                                        </li>
                                        <li>
                                            <a href="tel:0123456789">
                                                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                    <path opacity="0.12" d="M6.15153 8.21178C6.73153 9.41979 7.52219 10.552 8.5235 11.5533C9.52482 12.5546 10.657 13.3453 11.865 13.9253C11.9689 13.9752 12.0209 14.0001 12.0866 14.0193C12.3203 14.0874 12.6072 14.0385 12.805 13.8968C12.8607 13.8569 12.9083 13.8093 13.0036 13.714C13.2949 13.4227 13.4406 13.277 13.5871 13.1818C14.1395 12.8226 14.8516 12.8226 15.404 13.1818C15.5505 13.277 15.6962 13.4227 15.9875 13.714L16.1499 13.8764C16.5928 14.3193 16.8142 14.5407 16.9345 14.7785C17.1737 15.2515 17.1737 15.8101 16.9345 16.283C16.8142 16.5208 16.5928 16.7423 16.1499 17.1852L16.0186 17.3165C15.5772 17.7579 15.3565 17.9785 15.0565 18.1471C14.7236 18.3341 14.2065 18.4686 13.8246 18.4674C13.4805 18.4664 13.2453 18.3997 12.775 18.2661C10.2471 17.5487 7.86184 16.1949 5.87185 14.205C3.88187 12.215 2.52814 9.82968 1.81067 7.30185C1.67716 6.83148 1.61041 6.59629 1.60938 6.25217C1.60825 5.87032 1.74271 5.35324 1.92973 5.02032C2.09827 4.72029 2.31895 4.49961 2.7603 4.05826L2.89166 3.9269C3.33453 3.48403 3.55596 3.2626 3.79378 3.14231C4.26675 2.90308 4.82531 2.90308 5.29827 3.14231C5.53609 3.2626 5.75752 3.48403 6.20039 3.9269L6.36278 4.08929C6.65412 4.38063 6.79979 4.5263 6.89503 4.67278C7.25419 5.22519 7.25419 5.93734 6.89503 6.48975C6.79979 6.63623 6.65412 6.7819 6.36278 7.07324C6.26752 7.1685 6.21989 7.21613 6.18003 7.2718C6.03835 7.46966 5.98943 7.75655 6.05754 7.99017C6.0767 8.05592 6.10164 8.10787 6.15153 8.21178Z" fill="currentColor" />
                                                    <path d="M11.7094 4.99935C12.5233 5.15815 13.2713 5.55623 13.8577 6.14263C14.4441 6.72903 14.8422 7.47707 15.001 8.29102M11.7094 1.66602C13.4004 1.85388 14.9773 2.61116 16.1812 3.81352C17.3851 5.01588 18.1444 6.59186 18.3344 8.28268M8.5235 11.5519C7.52219 10.5506 6.73153 9.41839 6.15153 8.21037C6.10164 8.10647 6.0767 8.05451 6.05753 7.98877C5.98943 7.75514 6.03835 7.46826 6.18003 7.2704C6.21989 7.21472 6.26752 7.16709 6.36278 7.07183C6.65412 6.78049 6.79979 6.63483 6.89503 6.48834C7.25419 5.93593 7.25419 5.22378 6.89503 4.67138C6.79979 4.52489 6.65412 4.37923 6.36278 4.08789L6.20039 3.92549C5.75752 3.48263 5.53609 3.26119 5.29827 3.1409C4.8253 2.90168 4.26675 2.90168 3.79378 3.1409C3.55596 3.26119 3.33453 3.48263 2.89166 3.92549L2.7603 4.05686C2.31895 4.49821 2.09827 4.71889 1.92973 5.01891C1.74271 5.35183 1.60825 5.86891 1.60938 6.25076C1.61041 6.59488 1.67716 6.83007 1.81067 7.30044C2.52814 9.82827 3.88187 12.2136 5.87185 14.2036C7.86184 16.1935 10.2471 17.5473 12.775 18.2647C13.2453 18.3983 13.4805 18.465 13.8246 18.466C14.2065 18.4672 14.7236 18.3327 15.0565 18.1457C15.3565 17.9771 15.5772 17.7565 16.0186 17.3151L16.1499 17.1837C16.5928 16.7409 16.8142 16.5194 16.9345 16.2816C17.1737 15.8087 17.1737 15.2501 16.9345 14.7771C16.8142 14.5393 16.5928 14.3179 16.1499 13.875L15.9875 13.7126C15.6962 13.4213 15.5505 13.2756 15.404 13.1804C14.8516 12.8212 14.1395 12.8212 13.5871 13.1804C13.4406 13.2756 13.2949 13.4213 13.0036 13.7126C12.9083 13.8079 12.8607 13.8555 12.805 13.8954C12.6072 14.0371 12.3203 14.086 12.0866 14.0179C12.0209 13.9987 11.9689 13.9738 11.865 13.9239C10.657 13.3439 9.52482 12.5532 8.5235 11.5519Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                                  </svg>
                                                Call
                                            </a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- contact-area-end -->

        <!-- features-area -->
        <section class="features__area-three section-pb-120">
            <div class="container">
                <div class="features__inner-wrap-two" data-background="assets/img/bg/features_bg.jpg">
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="section__title text-center mb-50">
                                <h2 class="title">The Right Hosting Plan for Every Need</h2>
                                <p>Switch to a better host today! Migrate your website effortlessly with our expert support. Get started now!</p>
                            </div>
                        </div>
                    </div>
                    <div class="row gutter-y-24">
                        <div class="col-md-6">
                            <div class="features__item-five">
                                <div class="features__content-five">
                                    <h3 class="title">Effortless Managed WordPress Hosting</h3>
                                    <p>Get fast, secure, and hassle-free WordPress hosting with automatic updates, daily backups, and expert support.</p>
                                    <a href="hosting.html" class="tg-btn tg-border-btn-three">See Pricing
                                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M8.25 6.75L10.5 9L8.25 11.25" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                            <path d="M2.25 9C2.25 9.88642 2.42459 10.7642 2.76381 11.5831C3.10303 12.4021 3.60023 13.1462 4.22703 13.773C4.85382 14.3998 5.59794 14.897 6.41689 15.2362C7.23583 15.5754 8.11358 15.75 9 15.75C9.88642 15.75 10.7642 15.5754 11.5831 15.2362C12.4021 14.897 13.1462 14.3998 13.773 13.773C14.3998 13.1462 14.897 12.4021 15.2362 11.5831C15.5754 10.7642 15.75 9.88642 15.75 9C15.75 7.20979 15.0388 5.4929 13.773 4.22703C12.5071 2.96116 10.7902 2.25 9 2.25C7.20979 2.25 5.4929 2.96116 4.22703 4.22703C2.96116 5.4929 2.25 7.20979 2.25 9Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </a>
                                </div>
                                <div class="features__shape-three">
                                    <img src="assets/img/images/features_shape_icon01.svg" alt="shape">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                            <div class="features__item-five features__item-six">
                                <div class="features__content-five">
                                    <h3 class="title">Powerful & Scalable Cloud Hosting</h3>
                                    <p>Get fast, secure, and hassle-free WordPress hosting with automatic updates, daily backups, and expert support.</p>
                                    <a href="hosting.html" class="tg-btn tg-border-btn-three">See Pricing
                                        <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                                            <path d="M8.25 6.75L10.5 9L8.25 11.25" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                            <path d="M2.25 9C2.25 9.88642 2.42459 10.7642 2.76381 11.5831C3.10303 12.4021 3.60023 13.1462 4.22703 13.773C4.85382 14.3998 5.59794 14.897 6.41689 15.2362C7.23583 15.5754 8.11358 15.75 9 15.75C9.88642 15.75 10.7642 15.5754 11.5831 15.2362C12.4021 14.897 13.1462 14.3998 13.773 13.773C14.3998 13.1462 14.897 12.4021 15.2362 11.5831C15.5754 10.7642 15.75 9.88642 15.75 9C15.75 7.20979 15.0388 5.4929 13.773 4.22703C12.5071 2.96116 10.7902 2.25 9 2.25C7.20979 2.25 5.4929 2.96116 4.22703 4.22703C2.96116 5.4929 2.25 7.20979 2.25 9Z" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" />
                                        </svg>
                                    </a>
                                </div>
                                <div class="features__shape-three">
                                    <img src="assets/img/images/features_shape_icon02.svg" alt="shape">
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- features-area-end -->

    </main>
@endsection

@section('script')

@endsection